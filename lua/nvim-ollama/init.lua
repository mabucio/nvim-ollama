...
function M.open_floating_window()
	...
	if not M.persistent_buf or not vim.api.nvim_buf_is_valid(M.persistent_buf) then
		M.persistent_buf = vim.api.nvim_create_buf(false, true)
	end
	...
	if M.floating_win and vim.api.nvim_win_is_valid(M.floating_win) then
		...
		M.floating_win = vim.api.nvim_open_win(M.persistent_buf, true, opts)

		vim.api.nvim_create_autocmd("BufLeave", {
			buffer = M.persistent_buf,
			once = true,
			callback = function()
				if M.floating_win and vim.api.nvim_win_is_valid(M.floating_win) then
					vim.api.nvim_win_close(M.floating_win, true)
					M.floating_win = nil -- Reset handle so we can reopen it
				end
			end,
		})
	end
...
function M.stream_ollama(prompt)
	utils.append_to_buffer(M.persistent_buf, { "Asisstant: " })
	...
	vim.system({
		...
		stdout = function(_, data)
			...
			for line in data:gmatch("[^\r\n]+") do
				...
				if ok and decoded.response then
					...
					utils.append_to_buffer_text(M.persistent_buf, decoded.response)
				end
			end
		end,
		stderr = function(_, data)
			...
			if data then
				...
				return
				-- utils.append_to_buffer(M.persistent_buf, { data })
			end
		end,
	}, function()
		utils.append_to_buffer(M.persistent_buf, { "User: " })
		utils.move_cursor_to_end_of_buffer()
	end)
end
...
function M.ask_ollama_async(prompt)
	...
	vim.system({
		...
		if out.code ~= 0 then
			utils.append_to_buffer(M.persistent_buf, { "Error: Ollama request failed." })
			return
		end
		...
		if success and data.response then
			...
			utils.append_to_buffer(M.persistent_buf, lines)
		else
			utils.append_to_buffer(M.persistent_buf, { "Error: Failed to parse JSON." })
		end
	end)
	utils.append_to_buffer(M.persistent_buf, { "User: " })
	utils.move_cursor_to_end_of_buffer()
...
function M.setup()
	...
	M.persistent_buf = vim.api.nvim_create_buf(false, true)
	utils.append_to_buffer(M.persistent_buf, { "Initialized the plugin:" })
	vim.keymap.set("n", "<CR>", function()
		local input = utils.buf_to_str(M.persistent_buf)

		-- Don't send empty lines
		if input ~= "" then
			M.stream_ollama(input)
		else
			utils.append_to_buffer(M.persistent_buf, { "Empty input" })
		end
	end, { buffer = M.persistent_buf, desc = "Submit prompt to Ollama" })
